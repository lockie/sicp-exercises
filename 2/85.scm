...
(define (install-rational-package)
    ...
    (define (project x) (make-scheme-number (/ (numer x) (denom x))))
    (define (add-rat x y)
...
(define (install-polar-package)
    ...
    (define (make-from-real-imag x y)
        (cons (sqrt (+ (square x) (square y)))
              (atan y x)))
    (define (project x) (make-scheme-number (magnitude x)))
    ;; интерфейс к остальной системе
...
(define (install-complex-package)
    ...
    (put 'equ? '(complex rational)
         (lambda (z r) (equ-complex z (contents (make-complex-from-real-imag (/ (car r) (cdr r)) 0))))) ;ugh
    (put 'project 'complex (lambda (x) ((get 'project (type-tag x)) (contents x)))))
...
(define (drop x)
    (let ((project-proc (get 'project (type-tag x))))
        (if (not (null? project-proc))
            (let ((projected (project-proc (contents x))))
