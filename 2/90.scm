...
(define (install-dense-polynomial-package)
        ...
        (let ((term-order (order term))
            ...
            (cond ((=zero? (coeff term))
                  ...
                  ((= term-order (- list-length 1))
                   (adjoin-term-dense term (cdr term-list)))
                  ((< term-order list-length)
                   (cons (car term-list) (adjoin-term-dense term (cdr term-list))))
                  ((> term-order list-length)
    ...
    (put 'adjoin-term '(term dense)
         (lambda (t L) (tag (adjoin-term-dense (attach-tag 'term t) L))))
    (put 'first-term '(dense) first-term)
    (put 'rest-terms '(dense) (lambda (L) (tag (rest-terms L))))
    (put 'empty-termlist? '(dense) empty-termlist?)
    (put 'make-termlist 'dense (lambda () (tag '()))))

(define (make-dense-termlist)
    ((get 'make-termlist 'dense)))
...
(define (install-sparse-polynomial-package)
    ...
    (put 'adjoin-term '(term sparse)
         (lambda (t L) (tag (adjoin-term-sparse (attach-tag 'term t) L))))
    (put 'first-term '(sparse) first-term)
    (put 'rest-terms '(sparse) (lambda (L) (tag (rest-terms L))))
    (put 'empty-termlist? '(sparse) empty-termlist?)
    (put 'make-termlist 'sparse (lambda () (tag '()))))

(define (make-sparse-termlist)
    ((get 'make-termlist 'sparse)))
...
(display A) (newline)
(display B) (newline)
(display (sub B A)) (newline)

(define C (make-polynomial 'x (adjoin-term (make-term 100 1)
                                           (adjoin-term (make-term 1 1)
