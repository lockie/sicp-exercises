...
(#%require (only racket/base hash-remove!))
...
(define (make-new-machine)
  (let ((pc (make-register 'pc))
    ...
    (let ((the-ops
      ...
      (define (find-inst label)
          (define (find-inst-iter all-labels)
              (cond ((null? all-labels) nil)
                    ((null? (cdar all-labels)) nil)
                    ((eq? label (caar all-labels))
                     (caadar all-labels))
                    (else
                     (find-inst-iter (cdr all-labels)))))
          (find-inst-iter labels))
      (define (offset-inst inst offset)
          (define (offset-inst-iter insts n)
              (if (= n 0)
                  (car insts)
                  (offset-inst-iter (cdr insts) (- n 1))))
          (offset-inst-iter
      ...
      (define (execute)
          (let ((insts (get-contents pc)))
              (if (null? insts)
                  'done
                  (let* ((inst (car insts))
                         (bp (hash-ref breakpoints (instruction-text inst) nil)))
                      (if (not (null? bp))
                          (begin
                              (display (car bp))
                              (display " + ")
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'get-labels) (lambda () labels))
              ((eq? message 'set-labels) (lambda (l) (set! labels l)))
              ((eq? message 'proceed)
               (lambda ()
              ...
              ((eq? message 'set-breakpoint)
               (lambda (label offset)
                   (hash-set!
                    breakpoints
                    (offset-inst (find-inst label) offset)
                    (cons label offset))))
