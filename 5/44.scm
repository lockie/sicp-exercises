...
(define (overwrite? operator env)
    (eq? (find-variable operator env) '*not-found*))
...
(define (compile exp target linkage env)
  (cond ((self-evaluating? exp)
        ...
        ((open-code? exp env) (compile-open-code (operator exp) exp target linkage env))
        ((application? exp)
...
(define (compile-open-code opcode exp target linkage env)
    (end-with-linkage linkage
                      (preserving '(proc continue)
                                  ...
                                  (make-instruction-sequence
                                   '(arg1 arg2)
                                   (list target)
                                   `((assign ,target (op ,opcode) (reg arg1) (reg arg2)))))))

(define (spread-arguments operands env)
    ; NOTE : supporting only two operands
    (let ((first-operand (car operands))
          (second-operand (cadr operands)))
        (append-instruction-sequences
         (compile first-operand 'arg1 'next env)
         (compile second-operand 'arg2 'next env))))
