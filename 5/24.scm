      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'trace-off)
               (lambda () (set! trace #f)))
              ((eq? message 'get-register-tracing)
               (lambda (reg) (hash-ref registers-tracing reg #f)))
              ((eq? message 'set-register-tracing)
               (lambda (reg val)
                   (hash-set! registers-tracing reg val)))
  ...
  (let* ((reg-name (assign-reg-name inst))
    ...
    (let ((value-proc
      ...
      (lambda ()                ; execution procedure for assign
          (let ((val (value-proc)))
              (if ((machine 'get-register-tracing) reg-name)
                  (begin
                      (display "REGISTER TRACING: ")
                      (display reg-name)
                      (display ": ")
...
ev-cond-actions
  (restore exp)  ; the current clause
  (assign exp (op cond-actions) (reg exp))
  (assign exp (op car) (reg exp))
  (save continue)
  (assign continue (label ev-cond-actions-done))
  (save env)
  (goto (label eval-dispatch))

ev-cond-actions-done
  (restore env)
  (restore continue)
...
;((eceval 'trace-on))
;((eceval 'set-register-tracing) 'exp #t)
(start eceval)
