...
(define (=application? exp) (and (application? exp) (eq? (operator exp) '=)))
(define (*application? exp) (and (application? exp) (eq? (operator exp) '*)))
(define (-application? exp) (and (application? exp) (eq? (operator exp) '-)))
(define (+application? exp) (and (application? exp) (eq? (operator exp) '+)))
...
(define (compile exp target linkage)
  (cond ((self-evaluating? exp)
        ...
        ((=application? exp) (complie=application exp target linkage))
        ((*application? exp) (complie*application exp target linkage))
        ((-application? exp) (complie-application exp target linkage))
        ((+application? exp) (complie+application exp target linkage))
        ((application? exp)
...
(define (complie=application exp target linkage)
    (end-with-linkage linkage
                      (preserving '(proc continue)
...
(define (spread-arguments operands)
    ...
    (let ((first-operand (car operands))
        ...
        (append-instruction-sequences
         ...
         (compile second-operand 'arg2 'next))))
