...
(#%require (only racket/base hash-ref!))
(#%require (only racket/base hash-map))
...
(define (make-set)
    (let ((set '()))
        (define (dispatch m)
            (cond ((eq? m 'add)
                   (lambda (value)
                       (if (not (member value set))
                           (set! set (append (list value) set)))))
                  ((eq? m 'get) set)))
        dispatch))

(define *instructions* (make-hash))
(define *entrypoints* (make-set))
(define *saved-restored-regs* (make-set))
(define *regs-sources* (make-hash))
    ...
    (let ((the-ops
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'info) (list
                                    'instructions
                                    "\n"
                                    (hash-map
                                     *instructions*
                                     (lambda (x y) (list x " => " (y 'get) "\n")))
                                    "\n"
                                    'entrypoints
                                    "\n"
                                    (*entrypoints* 'get)
...
(define (make-execution-procedure inst labels machine
                                  pc flag stack ops)
  (hash-ref! *instructions* (car inst) make-set)
  (((hash-ref *instructions* (car inst)) 'add) inst)
...
(define (make-assign inst machine labels operations pc)
    (let* ((reg-name (assign-reg-name inst))
        ...
        (hash-ref! *regs-sources* reg-name make-set)
        (((hash-ref *regs-sources* reg-name) 'add) value-exp)
        (let ((value-proc
...
(define (make-save inst machine stack pc)
    (let* ((reg-name (stack-inst-reg-name inst))
           (reg (get-register machine reg-name)))
        ((*saved-restored-regs* 'add) reg-name)
         (lambda ()
