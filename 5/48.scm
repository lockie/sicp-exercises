...
(define (compile-and-run-call? exp)
    (and
     (application? exp)
     (eq? 'compile-and-run (operator exp))))

(define (compile-and-run expression)
    (assemble (statements
               (compile (cadar (operands expression)) 'val 'return))
              eceval))
...
(define eceval-operations
  (list
   ...
   (list 'compile-and-run-call? compile-and-run-call?)
   ))
...
eval-dispatch
  ...
  (test (op compile-and-run-call?) (reg exp))
