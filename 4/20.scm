...
(define (eval exp env)
    (cond ((self-evaluating? exp) exp)
          ...
          ((let? exp) (display (let->combination exp)) (newline) (eval (let->combination exp) env))
          ...
          ((application? exp)
           (apply-metacyclic (eval (operator exp) env)
                  (list-of-values (operands exp) env)))
          (else
           (error "Неизвестный тип выражения -- EVAL" exp))))

(define (apply-metacyclic procedure arguments)
...
(define (letrec->combination exp)
    (let ((vars (cadr exp)))
        (list 'let (map (lambda (v) (list (car v) (list 'quote '*unassigned*))) vars)
              (make-begin
               (append
                (map (lambda (v) (list 'set! (car v) (cadr v))) vars)
                (cddr exp))))))
