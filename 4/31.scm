...
(define (eval exp env)
    (cond ((self-evaluating? exp) exp)
          ...
          ((application? exp)
           (apply-metacyclic (actual-value (operator exp) env)
                  (operands exp)
                  env))
          (else
...
(define (energize-parameters parameters)
    (if (null? parameters)
        '()
        (let ((first (car parameters))
            ...
            (cond ((or (lazy-parameter? first)
                   ...
                   (cons (car first) (energize-parameters rest)))
                  ((variable? first)
                   (cons first (energize-parameters rest)))
                  (else (error "Bad syntax" first))))))

(define (apply-metacyclic procedure arguments env)
          ...
          ((compound-procedure? procedure)
           (let ((parameters (procedure-parameters procedure)))
               (eval-sequence
                ...
                (extend-environment
                 (energize-parameters parameters)
...
(define (delay-it param exp env)
    (cond ((lazy-parameter? param)
           (delay-lazy exp env))
          ((lazy-memo-parameter? param)
           (delay-lazy-memo exp env))
          (else (eval exp env))))
...
(define (list-of-delayed-args params exps env)
    (if (no-operands? exps)
        '()
        (cons (delay-it (car params) (first-operand exps) env)
              (list-of-delayed-args (cdr params) (rest-operands exps)
                                    env))))
...
(define input-prompt ";;; Ввод H-Eval:")

(define output-prompt ";;; Значение H-Eval:")
